...
- Языки: RU и EN. Все строки UI только через `useT()` и ключи из `src/i18n/ru.ts` (источник), `en.ts` зеркалит его; тест `i18n.test.ts` проверяет паритет ключей и плейсхолдеров. Домен слов не знает: `Verdict` это данные, слова подставляет `useVerdictLabel`. Язык: localStorage → navigator.language → en. Playwright ходит с `locale: 'ru-RU'`.
