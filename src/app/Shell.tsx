...
function NavItem({ to, icon, children, end }: { to: string; icon: string; children: ReactNode; end?: boolean }) {
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        cn(
          'flex min-h-11 items-center gap-2.5 rounded-md px-3 text-[15px]',
          isActive ? 'bg-surface font-medium text-text' : 'text-muted hover:bg-surface hover:text-text',
        )
      }
    >
    ...
    <div className="grid min-h-full md:grid-cols-[250px_minmax(0,1fr)]">
      <aside className="flex flex-col gap-5 border-b border-border bg-surface-side p-4 md:border-b-0 md:border-r md:px-5 md:py-6">
        <div className="flex flex-col gap-0.5">
          <span className="text-[13px] text-muted">Команда</span>
        ...
        {pending && (
          ...
          >
            <span className="font-semibold">{pending.sprint_name} закрыт в Jira?</span>
            <span>Загрузите CSV, и в калибровке появятся новые задачи.</span>
          </button>
        )}
        <button
          type="button"
          onClick={() => auth.signOut()}
          className="mt-auto flex min-h-11 items-center gap-2.5 rounded-md px-3 text-[15px] text-muted hover:bg-surface hover:text-text"
        >
          <Icon d={icons.logout} />
          Выйти
        </button>
